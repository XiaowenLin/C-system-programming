...
int trie_sum(trieNode_t *t, int isFirst)
{
	if (t == NULL) return 0;
	else if (*t->key == '\0') return 0;
	else
	{
		if (isFirst) return t->count + trie_sum(t->child, 0);
		else return t->count + trie_sum(t->next, 0) + trie_sum(t->child, 0);  
	}
}
...
void trie_allStr(queue_t *q, trieNode_t *t, int isFirst)
{	
	if (t == NULL || *t->key == '\0') return;
	if (isFirst == 0) 
	{
/*		printf("%s--->\n", t->str);	*/
		trie_allStr(q, t->next, 0);
	}
/*	printf("%s|||||\n", t->str);	*/
	trie_allStr(q, t->child, 0);
	if (t->count > 0)
	{
/*		printf("Enqueue %s\n", t->str);*/
		char *ret = malloc((strlen(t->str) + 1)*sizeof(char));
		strcpy(ret, t->str);
		queue_enqueue(q, ret);
	}
}
...
queue_t *trie_all_matches(trie_t *t, const char *prefix)
{
	...
	if (prefix[0] == '\0')
	{
/*		puts("---->detect empty prefix<-----");*/
		/* if empty prefix, return all*/
		queue_t *q = malloc(sizeof(queue_t));
		queue_init(q);
		trie_allStr(q, t->root->child, 0);
		return q;
	}
...
/*		puts("Both end");*/
/*		printf("node=%c, pref=%c\n", *matchNode->key, prefix[0]);*/
		size = strlen(matchNode->str) + 1;
		ret = malloc(size * sizeof(char));
		strcpy(ret, matchNode->str);
		return ret;
	}
	/* if match, next trie is \0, next prefix not, return null */
	if (matchNode != NULL && *matchNode->child->key == '\0' && prefix[1] != '\0')
...
/*		printf("node=%c, pref=%c\n", *matchNode->key, prefix[0]);*/
		...
		size = strlen(lnode.str) + 1;
		ret = malloc(size * sizeof(char));
		strcpy(ret, lnode.str);
		return ret;
	}	
	/* if no match, NULL*/
	else return NULL;
