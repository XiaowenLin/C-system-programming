...
void *malloc_and_break(void *region, int c, size_t len)
{
  ...
  printf("	[m_a_b] sr1 realloc(region=%p, %lu MB/ 3)\n", region, len/1024/1024);
  void *sr1 = realloc(region, len / 3);
  printf("	[m_a_b] sr2 malloc(%lu MB/ 3)\n", len/1024/1024);
  void *sr2 = malloc(len / 3);
  printf("	[m_a_b] sr3 malloc(%lu MB/ 3)\n", len/1024/1024);
  void *sr3 = malloc(len / 3);

  printf("	[m_a_b] verify_overlap3(sr1, sr2, sr3, len / 3)\n");
...
int main()
{
  ...
  while (len > MIN_ALLOC_SIZE) {
  	printf("<main> len=%lu MB ->malloc\n", len/1024/1024);
    void *mem = malloc(len);
    memset(mem, 0xff, len);
    printf("<main> ->free(malloc_and_break)\n");
